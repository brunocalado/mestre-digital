...
function gangUp() {
  ...
  let colorAlly = canvas.tokens.placeables.filter(t => 
  t.data.disposition === 1 
  && t.actor.data.data.status.isStunned === false 
  && withinRange(token, t, itemRange)
  );  
  Cast(colorAlly, false);
...
async function Cast(tokenList, actorType) {
  let actorColor;
  if (actorType) {
    actorColor = "modules/jb2a_patreon/Library/1st_Level/Cure_Wounds/CureWounds_01_Red_200x200.webm";
  } else {
    actorColor = "modules/jb2a_patreon/Library/1st_Level/Cure_Wounds/CureWounds_01_Blue_200x200.webm";
  }
  var arrayLength = tokenList.length;// Loops for multiple targets
  for (var i = 0; i < arrayLength; i++) {// Plays on targeted tokens
    let mainTarget = Array.from(tokenList)[i];// Scaled by Target Size
    let tarScale = ((mainTarget.data.width + mainTarget.data.height)/2);    // Defining spell animation for FX Master
    let spellAnim = 
                        {
                         file: actorColor,
